...
public interface ISignalementRepository extends CrudRepository<SignalementEntity, Long>{
    ...
    @Query(value = "SELECT dh_emissions, nb_annulations,nb_validations ,type_id,TRUNC(CAST(latitude AS NUMERIC), 4) AS latitude,TRUNC(CAST(longitude AS NUMERIC), 4) AS longitude,ID"
